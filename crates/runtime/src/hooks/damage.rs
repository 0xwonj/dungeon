...
impl PostExecutionHook for DamageHook {
    ...
    fn should_trigger(&self, ctx: &HookContext<'_>) -> bool {
        ...
        matches!(ctx.delta.action.kind, ActionKind::Attack(_))
...
mod tests {
    use super::*;
    use game_core::{AttackAction, AttackStyle, EntityId, GameState, StateDelta};

    use crate::oracle::OracleManager;

    #[test]
    fn triggers_for_attack_actions() {
        ...
        let delta = StateDelta::from_states(action, &state, &state);
    ...
    #[test]
    fn does_not_trigger_for_move_actions() {
        let state = GameState::default();
    ...
    #[test]
    fn chains_to_death_check() {
        let hook = DamageHook;
        assert_eq!(hook.next_hook_names(), &["death_check"]);
    }
}
