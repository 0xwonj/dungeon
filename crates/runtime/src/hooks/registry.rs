...
mod tests {
    use super::*;
    use crate::hooks::{HookContext, PostExecutionHook};
    ...
    impl PostExecutionHook for TestHook {
        ...
        fn should_trigger(&self, _ctx: &HookContext<'_>) -> bool {
            false
        }
    }

    #[test]
    fn sorts_hooks_by_priority() {
        let hooks: Vec<Arc<dyn PostExecutionHook>> = vec![
            Arc::new(TestHook {
                name: "third",
                priority: 10,
            }),
        ...
        let registry = HookRegistry::new(hooks.clone(), hooks);

        let names: Vec<_> = registry.hooks().map(|(name, _)| name).collect();
        assert_eq!(names, vec!["first", "second", "third"]);
    }

    #[test]
    fn default_registry_has_standard_hooks() {
        let registry = HookRegistry::default();
        assert_eq!(registry.len(), 2);

        let names: Vec<_> = registry.hooks().map(|(name, _)| name).collect();
        assert!(names.contains(&"action_cost"));
