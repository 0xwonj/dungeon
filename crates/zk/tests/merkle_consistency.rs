...
fn test_merkle_tree_simple() {
    ...
    println!("Proof depth: {}", proof.directions.len());
    for (i, (&sibling, &direction)) in proof
        .siblings
        .iter()
    ...
    let leaf_hash_var =
        poseidon_hash_many_gadget(&leaf_data_vars).expect("Failed to hash in circuit");
    let circuit_leaf_hash = leaf_hash_var
        .value()
        .expect("Failed to get leaf hash value");

    println!("Circuit leaf hash: {:?}", circuit_leaf_hash);
    println!("Leaf hashes match: {}", leaf_hash == circuit_leaf_hash);

    // Allocate proof path
    let path_vars: Vec<(FpVar<Fp254>, Boolean<Fp254>)> = proof
        .siblings
        .iter()
        .zip(proof.directions.iter())
...
fn test_merkle_tree_multiple_leaves() {
    ...
    assert!(
        verify_result.is_ok(),
        "Merkle path verification failed for leaf 1!"
    );
    assert!(cs.is_satisfied().unwrap(), "Constraints not satisfied!");
}
